...
def collect_samples(pid, queue, env, policy, custom_reward,
                    mean_action, render, running_state, min_batch_size, agent_count):
    for a in range(agent_count):
        torch.randn(pid*agent_count+a)
        log = dict()
        memory = Memory()
        num_steps = 0
        total_reward = 0
        ...
        if queue is not None:
            queue.put([pid*agent_count+a, memory, log])
            # print("pid {}. {}".format((pid-1)*agent_count+a+1, log['total_reward']))
        else:
            return memory, log
